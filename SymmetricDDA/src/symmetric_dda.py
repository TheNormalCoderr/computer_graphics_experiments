...
def display():
    ...
    # ── Compute Symmetric DDA points ─────────────────────────────────────
    x1, y1, x2, y2 = tc["x1"], tc["y1"], tc["x2"], tc["y2"]
    raw_pts = symmetric_dda_points(x1, y1, x2, y2)
    pts = unique_points(raw_pts)   # deduplicate for rendering

    # ── Direct line segments between consecutive pixels ───────────────────
...
# ───────────────────────────────────────────────────────────────────────────
def main() -> None:
    print("=== Symmetric DDA Line Drawing Algorithm (OpenGL) ===")

    glut.glutInit(sys.argv)
    glut.glutInitDisplayMode(glut.GLUT_SINGLE | glut.GLUT_RGB)

    # Generate colors dynamically based on the number of test cases
    global POINT_COLORS
    POINT_COLORS = _generate_colors(len(TEST_CASES))

    for idx, tc in enumerate(TEST_CASES):
        x1, y1, x2, y2 = tc["x1"], tc["y1"], tc["x2"], tc["y2"]
        dx    = x2 - x1
        dy    = y2 - y1
        m     = int(max(abs(dx), abs(dy)))
        steps = next_power_of_two(m)
        x_inc = round(dx / steps, 6) if steps else 0
        y_inc = round(dy / steps, 6) if steps else 0
        raw   = symmetric_dda_points(x1, y1, x2, y2)
        uniq  = unique_points(raw)
        print(f"[SymDDA] {tc['label']}  ({x1},{y1})\u2192({x2},{y2})  "
              f"dx={dx} dy={dy} 2^n={steps} x_inc={x_inc} y_inc={y_inc}  "
              f"raw={len(raw)} unique={len(uniq)}")

        glut.glutInitWindowSize(CANVAS_W, CANVAS_H)
        glut.glutInitWindowPosition(100 + idx * 40, 100 + idx * 40)

        win_title = f"Symmetric DDA - {tc['label']}".encode("utf-8")
